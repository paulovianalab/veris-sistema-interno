  ...
  return (
    <div className="p-8 md:p-12 max-w-[1600px] mx-auto space-y-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-border pb-8">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.4em] text-primary/60 mb-2">Visão Executiva</p>
          <h1 className="text-4xl font-black tracking-tight text-foreground">Dashboard</h1>
        </div>
        <div className="flex items-center gap-3">
           <div className="px-4 py-2 bg-muted/50 border border-border rounded-xl text-xs font-bold text-muted-foreground flex items-center gap-2">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        <div className="premium-card p-6 space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Clientes Ativos</span>
            <Users className="h-4 w-4 text-primary/50" />
          </div>
          <div>
            <div className="text-3xl font-black text-foreground">{activeClients}</div>
            <div className="flex items-center gap-1.5 mt-1">
              <span className="text-[10px] text-emerald-500 font-bold">Consolidado</span>
            </div>
          </div>
        </div>
        ...
        <div className="premium-card p-6 space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Novos Clientes (Mês)</span>
            <Target className="h-4 w-4 text-cyan-500/50" />
          </div>
          <div>
            <div className="text-3xl font-black text-foreground">{newClientsThisMonth}</div>
            <div className="flex items-center gap-1.5 mt-1">
              <span className={cn("text-[10px] font-bold", clientGrowth >= 0 ? "text-emerald-500" : "text-rose-500")}>
        ...
        <div className="premium-card p-6 bg-slate-50 dark:bg-slate-900 shadow-inner border-dashed">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Pendências</span>
            <CheckSquare className="h-4 w-4 text-orange-500/50" />
          </div>
          <div>
            <div className="text-3xl font-black text-foreground">{tasks.length}</div>
            <p className="text-[10px] text-muted-foreground font-bold mt-1">Tarefas em aberto</p>
          </div>
        </div>

        <div className="premium-card p-6 bg-primary text-primary-foreground border-none">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-black uppercase tracking-widest opacity-70">Receita Mensal (Fee)</span>
        ...
        <div className="lg:col-span-8 space-y-8">
          <div className="premium-card overflow-hidden">
            <div className="p-6 border-b border-border/40 flex items-center justify-between bg-muted/5">
              ...
              <a href="/clientes" className="text-[10px] font-black text-primary hover:underline uppercase tracking-widest">Gerenciar</a>
            </div>
            <div className="divide-y divide-border/30">
              {clients.slice(0, 5).map(client => (
                <div key={client.id} className="flex items-center justify-between p-5 hover:bg-muted/30 transition-colors">
                  <div className="flex items-center gap-4">
                    <div className="h-10 w-10 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center font-black text-primary border border-border/50 text-xs">
                      {client.company?.[0]?.toUpperCase() || client.name[0].toUpperCase()}
                    </div>
                    <div>
                      <p className="font-black text-foreground text-sm leading-none mb-1">{client.company || client.name}</p>
                      <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest">{client.name}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-8">
                    <div className="text-right hidden sm:block">
                      <p className="text-xs font-black text-foreground">R$ {client.monthlyValue.toLocaleString('pt-BR')}</p>
                      <p className="text-[9px] text-muted-foreground font-bold uppercase">Mensal</p>
                    </div>
                    {getStatusBadge(client.type)}
                  </div>
                </div>
              ))}
              {clients.length === 0 && <p className="p-10 text-center text-muted-foreground text-sm font-bold">Nenhum cliente disponível.</p>}
            </div>
          </div>

          <div className="premium-card p-8">
             <div className="mb-6">
                <h3 className="text-sm font-black uppercase tracking-widest mb-1">Crescimento de Atividade</h3>
             ...
             <div className="h-48 w-full flex items-end justify-between gap-3 px-4">
                {[40, 65, 45, 90, 55, 75, 85].map((h, i) => (
                  <div key={i} className="flex-1 group relative">
                    ...
                    />
                    {i === 3 && <div className="absolute -top-6 left-1/2 -translate-x-1/2 text-[10px] font-black text-primary">PICO</div>}
                  </div>
                ))}
             </div>
             <div className="flex justify-between mt-4 text-[9px] font-black text-muted-foreground uppercase tracking-widest px-4 opacity-50">
        ...
        <div className="lg:col-span-4 space-y-8">
          <div className="premium-card overflow-hidden">
            ...
            <div className="p-6 space-y-6">
              {tasks.map(task => (
                <div key={task.id} className="flex gap-4 group">
                  ...
                  <div className="space-y-1">
                    <p className="text-xs font-black text-foreground group-hover:text-primary transition-colors leading-tight">
                      {task.title}
                    </p>
              ...
              {tasks.length === 0 && <p className="text-center text-muted-foreground text-xs font-bold py-10 italic">Nenhuma atividade pendente.</p>}
              <button className="w-full py-3 bg-muted hover:bg-muted/80 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">
          ...
          <div className="p-8 rounded-3xl bg-slate-900 text-white space-y-6 relative overflow-hidden group">
             <div className="relative z-10">
                <h3 className="text-xs font-black uppercase tracking-widest opacity-60">Operação Veris</h3>
                <p className="text-xl font-black mt-2 leading-tight">Mantenha o foco na entrega de alto nível.</p>
