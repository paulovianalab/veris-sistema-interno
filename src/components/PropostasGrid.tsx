...
// Safe date formatter
function formatProposalDate(dateStr: any): string {
  try {
...
// Safe proposal card component
function ProposalCard({ proposal, onEdit }: { proposal: any; onEdit: (p: any) => void }) {
  if (!proposal || !proposal.id) {
    return null;
  }

  const status = proposal.status || "Pendente";
  const title = proposal.title || "Sem título";
  const clientName = proposal.client?.company || proposal.client?.name || "Sem cliente";
  const value = proposal.value || 0;

  const getStatusBadge = () => {
    switch (status) {
      case "Aprovada":
        return <Badge variant="success" className="text-[10px] font-medium uppercase px-2 py-0.5">Aprovada</Badge>;
      case "Enviada":
        return <Badge variant="info" className="text-[10px] font-medium uppercase px-2 py-0.5">Enviada</Badge>;
      case "Recusada":
        return <Badge variant="destructive" className="text-[10px] font-medium uppercase px-2 py-0.5">Recusada</Badge>;
      default:
        return <Badge variant="secondary" className="text-[10px] font-medium uppercase px-2 py-0.5">{status}</Badge>;
    }
  };

  return (
    <div className="group relative flex flex-col gap-6 rounded-[2.5rem] border border-border bg-card p-8 hover:border-primary/30 transition-all duration-500 shadow-xl hover:shadow-2xl hover:shadow-primary/5 hover:-translate-y-1">
